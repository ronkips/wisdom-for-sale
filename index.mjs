...
console.log(`The consensus network ${stdlib.connector}.`);

const suStr = stdlib.standardUnit;
const auStr = stdlib.atomicUnit;

const toAU = (su) => stdlib.parseCurrency(su);
const toSU = (au) => stdlib.parseCurrency(au, 4);
...
if (role=== 'seller') {
    ...
    const ctc = acc.contract(backend); //get reference to the contract 
    await ctc.participants.Seller(sellerInteract);
    await showBalance(acc); //initiate interaction with the contract for seller
